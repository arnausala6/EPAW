	...
	<div class="w3-container w3-padding">
		<form action="EditProfile" method="POST" enctype="multipart/form-data">
			<div class="w3-section">
				...
				<input class="w3-input w3-border w3-light-grey" type="text" id="editUsername" name="username" maxlength="30" value="${user.username}" readonly>
			</div>
			...
			<div class="w3-section">
				...
				<select class="w3-input w3-border" id="editGender" name="gender">
					<option value="">—</option>
					<option value="male"   ${user.gender == 'male'   ? [cite_start]'selected' : ''}>Male</option> [cite: 2]
					<option value="female" ${user.gender == 'female' ? 'selected' : ''}>Female</option>
					<option value="other"  ${user.gender == 'other'  ? [cite_start]'selected' : ''}>Other</option> [cite: 3]
				</select>
			</div>
			...
			<div class="w3-section">
				...
				<select class="w3-input w3-border" id="country" name="country">
					...
					<c:forEach var="c" items="${countries}">
						<option value="${c}" ${user.country == c ? [cite_start]'selected' : ''}>${c}</option> [cite: 4]
					</c:forEach>
				</select>
			</div>
			...
			<div>
				<button type="submit" class="w3-button w3-theme">Save changes</button>
